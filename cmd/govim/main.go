...
type govimplugin struct {
	...
	// lastDiagnosticsChangedQuickfix records the last diagnostics that were used
	// when updating the quickfix window
	lastDiagnosticsChangedQuickfix *[]types.Diagnostic

	// lastDiagnosticsChangedSigns records the last diagnostics that were used when
	// updating signs
	lastDiagnosticsChangedSigns *[]types.Diagnostic

	// lastDiagnosticsChangedHighlights records the last diagnostics that were used
	// when updating highlights
	lastDiagnosticsChangedHighlights *[]types.Diagnostic
...
func (g *govimplugin) Init(gg govim.Govim, errCh chan error) error {
	...
	g.DefineAutoCommand("", govim.Events{govim.EventBufNew}, govim.Patterns{"*"}, false, g.vimstate.bufNew, "eval(expand('<abuf>'))", "fnamemodify(expand('<afile>'),':p')")
	...
	g.DefineAutoCommand("", govim.Events{govim.EventBufWinEnter}, govim.Patterns{"*"}, false, g.vimstate.bufWinEnter, exprAutocmdCurrBufInfo)
	g.DefineAutoCommand("", govim.Events{govim.EventBufUnload}, govim.Patterns{"*"}, false, g.vimstate.bufUnload, "eval(expand('<abuf>'))")
	g.DefineAutoCommand("", govim.Events{govim.EventBufWritePre}, govim.Patterns{"*"}, false, g.vimstate.formatCurrentBuffer, "eval(expand('<abuf>'))")
	...
	g.DefineAutoCommand("", govim.Events{govim.EventUser}, govim.Patterns{"PostInitComplete"}, true, g.vimstate.postInitCompleteAutocommand, postInitCompleteExpr)
	g.defineHighlights()
...
func (v *vimstate) postInitCompleteAutocommand(args ...json.RawMessage) error {
	var bufInfos []bufReadDetails
	v.Parse(args[0], &bufInfos)
	for _, b := range bufInfos {
		v.bufNewImpl(b.Num, b.Name)
	}
	for _, b := range bufInfos {
		if b.Loaded == 1 {
			v.bufWinEnterImpl(b)
