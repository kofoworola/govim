...
// buffer (file), followed by BufDelete of the old unnamed buffer, followed by
// BufDelete of the new buffer (file), followed by another BufNew for the new
// buffer (file). Don't ask me why
// 2. when setting a quickfix list for a file number of occasions when we
// BufCreate is then the point at which we add a listener to the buffer. Buf
func (v *vimstate) bufNew(args ...json.RawMessage) error {
	bufnr := v.ParseInt(args[0])
	bufname := v.ParseString(args[1])
	return v.bufNewImpl(bufnr, bufname)
}

func (v *vimstate) bufNewImpl(bufnr int, bufname string) error {
	if _, ok := v.buffers[bufnr]; ok {
		return fmt.Errorf("we already know about buffer %v; how is that possible?", bufnr)
	}
	v.buffers[bufnr] = &types.Buffer{Num: bufnr, Name: bufname}
	return nil
}
...
// a buffer being loaded. The corresponding lifecycle event is BufUnload
...
func (v *vimstate) bufWinEnterImpl(nbinfo bufReadDetails) error {
...
// to work around some Vim issues documented elsewhere in this file.
